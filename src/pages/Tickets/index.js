...
const Index = () => {
  ...
  const getRatingDetails = async () => {
    try {
      LoaderShow();
      const response = await postAPI("rating/get-rating");
      if (response.statusCode === 200) {
        LoaderHide();
  ...
  const validationType = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ratingDescription: Yup.string()
        ...
        .min(3, "Rating description must be at least 3 characters long"),
      ratingValues: Yup.number()
        .min(1, "Please provide at least one star rating")
        .required("Rating is required"), // ensures rating is provided
    }),
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const data = {
          ...
          ratingValues: values?.ratingValues,
          ticketName: ticketRating?.ticketName,
          ticketID: ticketRating?.ticketID,
          userId: ticketRating?.userId,
          customerId: ticketRating?._id,
        };
        LoaderShow();
        const response = await postAPI("rating/add-rating", data);
        if (response.statusCode === 200) {
          resetForm();
          toast.success(response.message);
          LoaderHide();
  ...
  const handleCancel = () => {
    toggleRatingModal();
    // Reset specific fields or the entire form
    validationType.setFieldValue("ratingDescription", ""); // Clear the rating description
    validationType.setFieldValue("ratingValues", 0); // Reset the rating to 0 (no rating selected)
  };
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="page-content">
                                    ...
                                    {column.key === "ticketID" ? (
                                      <>
                                        {location?.pathname ===
                                          ...
                                          <Link
                                            to={`/default/ticket-details/${row.ticketID}`} // Default link, can adjust if needed
                                            state={{ ticketData: row }} // Passing only the clicked row's data
                                            style={{
                                              textDecoration: "none",
                                              color: "#ed8d21",
                                            ...
                                            title={
                                              ratingDetails.find(
                                                (rating) =>
                                                  rating.ticketID ===
                                                  row.ticketID
                                              )?.ratingDescription || "" // Assuming you might want to change this to something dynamic later
                                            }
                                            placement="top"
                                          >
                                            <span>
                                    ...
                                    <Col lg={6}>
                                      <div className="mb-3">
                                        {ratingDetails.some(
                                          ...
                                          <Tooltip
                                            title={
                                              // ticketRating?.description // assuming description is available in the response
                                              //   ? ticketRating.description
        ...
        {/* Rating Modal Starts */}
        <Modal isOpen={ratingModal} toggle={toggleRatingModal}>
          <ModalHeader toggle={toggleRatingModal}>
            Please Add Rating for Ticket ID : {ticketRating?.ticketID}
          </ModalHeader>
          <ModalBody>
            <Row className="mb-4">
              <Col lg={6}>
                <div className="mb-3">
                  <Rating
                    ...
                    onChange={(event, value) =>
                      validationType.setFieldValue("ratingValues", value)
                    }
                    onBlur={validationType.handleBlur}
                    value={validationType.values.ratingValues || ""}
                  />
                  <br />
                  {validationType.touched.ratingValues &&
                    validationType.errors.ratingValues ? (
              ...
              <Col lg={6}>
                <div className="mb-3">
                  ...
                  <Input
                    ...
                    onBlur={validationType.handleBlur}
                    value={validationType.values.ratingDescription || ""}
                  />
                  {validationType.touched.ratingDescription &&
                    validationType.errors.ratingDescription ? (
                    <span style={errorStyle}>
                      {validationType.errors.ratingDescription}
                    </span>
                  ) : null}
...
const DropdownComponent = ({ row, confirmDelete }) => {
  ...
  const goToEditPage = (ticketData) => {
    // Extract only the base path ("/customer/ticket-details")
    ...
    } else if (basePath === "/customer/tickets") {
  ...
  const handleDeleteTicket = (row) => {
    const { status, ticketID } = row;

    if (status === "accepted" && basePath === "/customer/tickets") {
      // Customer cannot delete an "accepted" ticket
      toast.error("You cannot delete an accepted ticket.");
    } else if (
      status === "created" ||
      status === "in-progress" ||
      status === "rejected" ||
      (status === "accepted" && basePath === "/admin/tickets")
    ) {
      // Both can delete tickets with specific statuses; only admin can delete "accepted" tickets
  ...
  return (
    <Dropdown className="action" isOpen={dropdownOpen} toggle={toggleDropdown}>
      ...
      <DropdownMenu>
        ...
        <DropdownItem onClick={() => handleDeleteTicket(row)}>
          Delete
        </DropdownItem>
      </DropdownMenu>
    </Dropdown>
