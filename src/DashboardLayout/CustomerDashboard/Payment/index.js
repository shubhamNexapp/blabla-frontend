  ...
  const filteredData = data
    .filter((row) => {
      return (
        ...
        row?.totalPayment.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const paymentHandler = async (selectedRow) => {
    try {
      ...
      const paymentData = {
        ...
        // Service Price + (Service Price * GST%) + Platform Fee
        amount: 100 * parseInt(selectData[0].totalPayment),
        currency: "INR",
        receipt: selectData[0].ticketID,
      };
      const ticketId = {
      ...
      var options = {
        ...
        prefill: {
          ...
          name: selectData[0].userId, //your customer's name
          email: selectData[0].userId,
                ...
                <Row>
                  <Col xl={12}>
                    <div className="table-settings">
                      {/* {selectedRows.length ? (
                                                <button onClick={() => paymentHandler(selectedRows)} className="me-2 btn btn-danger">
                                                    Pay
                                                </button>
                                            ) : null} */}
                      {/* {selectedRows.length ? (
                          ...
                          <tr style={{ textAlign: "center" }} key={row._id}>
                                    ...
                                    {column.key === "ticketID" ? (
                                      <>
                                        ...
                                        "/admin/payment" ? (
                                          ...
                                          "/customer/payment" ? (
                                          <Link
                                            to={`/customer/ticket-payment-details/${row.ticketID}`} // Link to the ticket details page for customer
                                            state={{ ticketData: row }} // Passing only the clicked row's data
                                            style={{
                                              textDecoration: "none",
                                              color: "#ed8d21",
                            ...
                            <td>
                              {row.paymentstatus === "pending" ? (
                                <button
                                  onClick={() => paymentHandler(row._id)}
                                  className="me-1 btn btn-warning"
                                >
                                  Pay
                                </button>
                              ) : null}
                              {/* <button style={{ fontSize: '13px', color: 'blue' }}
                                                                onClick={() => paymentHandler(row._id)} className="me-1 btn btn-link">
                                                                Pay
                                                            </button> */}
                            </td>
                          </tr>
